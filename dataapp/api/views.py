...
@blueprint.route('/submission', methods=['POST'])
def add_submission():
    json_input = request.json()
    new_sub = Submission(json_input.id,
                         json_input.script_name,
                         json_input.simulation_name,
                         json_input.script_version,
                         json_input.simulation_version)
    ...
    return jsonify(new_sub.serialize())
...
def simulations(sub_num=None, page_num=1):
    ...
    list_of_sims = [s.serialize() for s in sim]

    return jsonify(list_of_sims)
